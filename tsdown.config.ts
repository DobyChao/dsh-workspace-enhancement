/**
 * Client bundle build for dsh-workspace-enhancement (mirrors the harness
 * `tsdown.client.ts` preset): emits the browser artifact as a CJS closure
 * registered through `window.__ModuleLoader__.load`, resolves platform modules
 * (react, cordis, ui-slots, …) from the loader module table, inlines everything
 * else, and compiles CSS Modules with lightningcss into self-injecting
 * stylesheets.
 */
...
const ID = 'dsh-workspace-enhancement'
