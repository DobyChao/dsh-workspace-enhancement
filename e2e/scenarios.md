...
| E2E-05 | 副工作区面板 + 主题跟随 | `REQ-I3`（单 session 多工作区/副目录 + 逐工作区权限）；`UX-2`（面板「浏览」输入框）；`INFRA-5` | `specs/E2E-05-side-workspaces-panel.spec.ts` | ① 会话标题栏出现「工作区」按钮（`title=side.headerAction.title`，文本 = `side.headerAction.label`）；② 点击后 `role=dialog` 面板打开，标题/本机目录/远程目录/浏览…/挂载/两个权限下拉（`fs 权限`、`执行权限`）齐全；③ 新会话显示空态文案（`side.empty`）；④ 面板解析 `--dswsw-surface` 主题 token、卡片背景不透明且等于该 token、文字对比度 ≥ 3:1；⑤ light/dark 下背景亮度方向正确（lab 主题被钉死时记 `theme-pinned` annotation 并跳过方向断言）；⑥ 关闭按钮可关闭面板 |
