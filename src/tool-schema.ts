...
const SW_EXEC_DESCRIPTION = 'Execute a command on a registered SSH server connected to this session and return its stdout/stderr. The `server` id selects the machine — it must be a registry id connected to this session (see the connection line of sw_status, or call sw_connect first). If this session\'s main workspace is already that remote machine, use bash instead of sw_exec. On Windows, local paths use pwsh, not sw_exec. On Linux, server "local" runs on this host only while the main workspace is remote. The target OS is probed once per connection and reported in the first line: POSIX runs `bash -c`, Windows runs `pwsh -Command`, unknown runs bash honestly. Each call runs in a fresh shell: no state (cwd, variables, functions) persists between calls — pass `workdir` instead of using `cd`. Non-zero exits are reported as `[exit code: N]` — investigate failures before moving on. Long output is truncated to its tail; the full output is saved to a file whose path is reported when available.'

const BASH_DESCRIPTION = 'Execute a bash command (`bash -c`) on the session\'s remote Linux workspace and return its stdout/stderr. It stays on this session\'s remote machine: a local path or another server is refused (pwsh for local Windows commands, sw_exec for another server). Each call runs in a fresh shell: no state (cwd, variables, functions) persists between calls — pass `workdir` instead of using `cd`. Non-zero exits are reported as `[exit code: N]` — investigate failures before moving on. Long output is truncated to its tail; the full output is saved to a file whose path is reported when available.'
...
export function swExecParams(backgroundEnabled: boolean) {
  return {
    ...
    server: {
      type: 'string' as const,
      description: 'Target server id: a registry machine id (c1, c2, …) connected to this session, other than this session\'s own remote machine (use bash for that one). "local" is reserved: on Linux it runs on this host when the main workspace is remote; on Windows it is refused (use pwsh). A registered id that is not connected to this session errors with the connected ids; an unknown id errors with the known list.',
    },
