...
`danger-full-access`，不进 bwrap）。无核心且本次仍是围栏档 → **fail-closed**，
不退 SFTP。无核心且本次是 danger → 今天的 SFTP + 裸 SSH。
...
1. **运输 vs 权限**
   - 运输：核心探针通过 → 走 RPC。失败且本次围栏档 → `SANDBOX_UNAVAILABLE`（fs 与
     spawn 一起）。失败且本次 danger → SFTP/SSH。
...
带提权标记；提权允许后成功；无核心 + workspace-write 不走 SFTP。
