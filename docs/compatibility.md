...
| 2026-09-25（**`0.1.7-rc.2` 接缝再扩**） | `next` 推进到 `0.1.7-rc.2`：① `dsh-fs-local` 具体后端新增 `watch(target, changed, signal)`（chokidar，基类未声明）；② subprocess 的 `terminalEnvironment` 落到具体本地后端 | 手动 dispatch 的 drift 双通道红：`MixedFileSystem` 缺 `watch` 被**反射契约**点名；`MixedSubprocessRuntime` 缺 `terminalEnvironment` **无任何门拦住**——R14 O2 预言的「门面=子集类型」在 subprocess 侧没有反射契约（→ `AUDIT-1` 补） | **已修（`UPSTREAM-5`）**：fs 门面补 `watch`（local 转发 / remote 诚实 `FS_IO_ERROR` / 老委托同式守卫）；subprocess 门面补 `terminalEnvironment`（世界无关随 local 委托，委托太老按上游同式自算宿主事实）；契约静态清单扩到 15 方法 | `test/mixed-fs-contract.test.ts`（15 方法全集）+ drift 双通道（typecheck + 单测 + boot smoke）+ subprocess 反射契约（`AUDIT-1`） |
