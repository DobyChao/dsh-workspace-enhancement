...
| UPSTREAM-1 | 上游 alpha 新增 `FileSystem.readByteRange` → 并入 rc 前需实现 | todo | P2 | 2026-09-09 由双通道哨兵抓到（`drift (alpha)` 红、`drift (next)` 绿）：alpha `0.1.5-alpha.2` 给上游 `FileSystem` 加了抽象成员 `readByteRange`，我们的 `SshFileSystem extends FileSystem`（`src/filesystem.ts:589`）未实现 → TS2515；同批还引入新包 `dsh-brand` / `dsh-invariants`。**不是现在必须改**（alpha 未并入 rc），但要在它变成 rc 之前完成：给 `SshFileSystem` 实现 `readByteRange`（转发 `SshFileSystemEngine`），并评估 `MixedFileSystem` / `FileSystemBranch`（`src/mixed.ts:190`）是否同步补第 14 个方法；跟踪 issue #7。验收：`upstream.yml` 的 `drift (alpha)` 转绿 |
