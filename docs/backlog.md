...
| BUG-10 | 远程项目 skill 目录包发现失败（win32 join 搅碎 `ssh://` 前缀） | todo | P1 | win32 join 把 displayPath 搅碎成 `.\ssh:\<id>…`，不以 `ssh://` 开头，resolve 落本地静默跳过（R38 实证）。分支两层修：`parseSshRoute` 归一路径 `\`；`remoteRouteFromCwd` 重建搅碎拼写。待 R38 复跑。根因与走查 [R38](./uat/R38-bug10-skill-dir-uat.md) §4 |
