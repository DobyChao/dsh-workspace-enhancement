...
| ID | 标题 | 状态 | 优先级 | 备注 |
|---|---|---|---|---|
| BUG-5 | ssh2 Client 生命周期无 error 监听：ready 后 ECONNRESET 打挂宿主进程 | todo | P1 | 已核实属实（issue #19，2026-09-16 对 master 复核）。根因：`connectReady()`（src/ssh-core.ts:158-170）ready 一到即摘掉唯一 error 监听，此后全仓库对 ssh2 `Client` 无任何 error 监听；`keepaliveInterval` 默认 0（runtime.ts:178、connection.ts:278）→ 空闲回收/keepalive 超时以裸 ECONNRESET 冒出 → Node 对无人监听的 'error' 抛未捕获异常 → **整个 dsh web 宿主退出**（非仅插件失效）。同源缺陷：`isConnected()`（ssh-core.ts:529）只在 dispose 翻 false，静默断链后 registry/UI 仍报「已连接」。修法认 issue #19：① 必做——`openChain()` 内 `new Client()` 后立即挂生命周期 error 监听（标记失效、清 ready/sftp 缓存使下次调用重连）；② keepalive 默认改正值（如 30000×3；行为变更，需所有者拍板）；③ 修 isConnected 语义让状态点反映断链。验收：ready 后强杀连接（ss -K / 停 sshd）宿主进程存活、状态点转离线、下次操作自动重连 |
