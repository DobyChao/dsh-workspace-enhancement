...
import { assertLinuxAmd64, coreInstallScript } from '../src/core-deploy.ts'
...
test('coreInstallScript: chmod includes dsh-core and bundled bin helpers', () => {
  const script = coreInstallScript('0.2.0-dev', '/tmp/dsh-core-0.2.0-dev-linux-x64.tar.gz')
  assert.match(script, /chmod \+x -- .*\/dsh-core .*\/bin\/bwrap .*\/bin\/rg/)
})
